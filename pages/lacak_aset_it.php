...
if (isset($_GET['search'])) {
    ...
    $sql = "
        SELECT a.id, a.no_inventaris, a.nama_aset, a.kategori, a.merek, a.model_aset,
               ...
               u.nama AS pj_nama, u.id AS pj_id
        FROM aset_it a
    ...
    if ($pic > 0) {
        // Filter by PIC only
        $sql .= " AND a.pj_user_id = ?";
        $params[] = $pic;
    } else {
        // Full-text search
        $sql .= " AND (
            a.no_inventaris LIKE ? OR a.nama_aset LIKE ?
    ...
    $sql .= " ORDER BY a.nama_aset ASC LIMIT 50";
    $st = $pdo->prepare($sql);
    $st->execute($params);
...
// ── API: Daftar PIC (untuk dropdown) ─────────────────────────────────────────
if (isset($_GET['pic_list'])) {
    $st = $pdo->query("
        SELECT u.id, u.nama, u.divisi,
               COUNT(a.id) AS jumlah_aset
        FROM users u
        INNER JOIN aset_it a ON a.pj_user_id = u.id
        GROUP BY u.id, u.nama, u.divisi
        ORDER BY jumlah_aset DESC, u.nama ASC
    ");
    header('Content-Type: application/json');
    echo json_encode($st->fetchAll(PDO::FETCH_ASSOC));
    exit;
...
.lk-tab:hover { border-color: #00c896; color: #00b386; background: #f0fdf9; }
.lk-tab.active { background: #00c896; border-color: #00c896; color: #fff; }
.lk-tab i { font-size: 11px; }
...
/* PIC filter panel */
.lk-pic-panel { display: none; }
.lk-pic-panel.show { display: block; }

.lk-pic-search-row { display: flex; align-items: center; gap: 10px; margin-bottom: 12px; }
.lk-pic-search-inp {
    flex: 1; height: 40px; padding: 0 14px 0 38px;
    border: 1.5px solid #e2e8f0; border-radius: 8px;
    background: #f8fafc; color: #1e293b;
    font-size: 13px; font-family: 'Inter', sans-serif; font-weight: 500;
    outline: none; transition: all .15s;
}
.lk-pic-search-inp:focus { border-color: #6366f1; background: #fff; box-shadow: 0 0 0 3px rgba(99,102,241,.08); }
.lk-pic-search-inp::placeholder { color: #94a3b8; font-weight: 400; }
...
.lk-pic-grid::-webkit-scrollbar { width: 3px; }
.lk-pic-grid::-webkit-scrollbar-thumb { background: #e2e8f0; border-radius: 2px; }
...
.lk-pic-card:hover { border-color: #6366f1; background: #fafbff; }
.lk-pic-card.selected { border-color: #6366f1; background: #eef2ff; }
.lk-pic-card.selected::before {
...
.lk-pic-name { font-size: 12px; font-weight: 700; color: #1e293b; line-height: 1.3; }
.lk-pic-divisi { font-size: 10.5px; color: #94a3b8; margin-top: 1px; }
.lk-pic-badge {
...
.lk-pic-card.selected .lk-pic-badge { background: #6366f1; color: #fff; }
...
.lk-selected-pic.show { display: flex; }
.lk-selected-pic-ava {
    width: 28px; height: 28px; border-radius: 50%;
    display: flex; align-items: center; justify-content: center;
    font-size: 11px; font-weight: 800; color: #fff; flex-shrink: 0;
}
.lk-selected-pic-name { font-size: 12.5px; font-weight: 700; color: #3730a3; }
.lk-selected-pic-sub  { font-size: 10.5px; color: #6366f1; }
.lk-selected-pic-clr  { margin-left: auto; }
.lk-selected-pic-clr button {
...
.lk-selected-pic-clr button:hover { background: #c7d2fe; }
...
/* PIC summary di atas list saat mode PIC */
.lk-pic-summary {
    display: none; padding: 10px 14px; background: #fafbff;
    border-bottom: 1px solid #e2e8f0; gap: 10px; align-items: center;
}
.lk-pic-summary.show { display: flex; }
.lk-pic-sum-ava {
    width: 32px; height: 32px; border-radius: 50%;
    display: flex; align-items: center; justify-content: center;
    font-size: 12px; font-weight: 800; color: #fff; flex-shrink: 0;
}
.lk-pic-sum-name { font-size: 12.5px; font-weight: 700; color: #1e293b; }
.lk-pic-sum-sub  { font-size: 10.5px; color: #6366f1; }
.lk-pic-sum-badge {
    margin-left: auto; flex-shrink: 0;
    font-family: 'JetBrains Mono', monospace; font-size: 11px; font-weight: 700;
    color: #6366f1; background: #eef2ff; padding: 2px 8px; border-radius: 8px;
}
...
@media (max-width: 560px)  { .lk-stats { grid-template-columns: repeat(2,1fr); } .lk-pic-grid { grid-template-columns: 1fr 1fr; } }
</style>
...
<div class="lk">
    ...
    <div class="lk-search-header">
        ...
        <!-- Mode Tabs -->
        <div class="lk-mode-tabs">
            <button class="lk-tab active" id="tab-aset" onclick="switchMode('aset')">
                <i class="fa fa-magnifying-glass"></i> Cari Aset
            </button>
            <button class="lk-tab" id="tab-pic" onclick="switchMode('pic')">
                <i class="fa fa-user"></i> Berdasarkan PIC
            </button>
        </div>
        ...
        <!-- MODE: Filter PIC -->
        <div id="panel-pic" class="lk-pic-panel">

            <!-- Selected PIC bar -->
            <div class="lk-selected-pic" id="selected-pic-bar">
                <div class="lk-selected-pic-ava" id="spic-ava"></div>
                <div>
                    <div class="lk-selected-pic-name" id="spic-name"></div>
                    <div class="lk-selected-pic-sub" id="spic-sub"></div>
                </div>
                <div class="lk-selected-pic-clr">
                    <button onclick="clearPIC()"><i class="fa fa-times"></i> Ganti PIC</button>
                </div>
            </div>

            <!-- PIC Search -->
            <div class="lk-pic-search-row" id="pic-search-row">
                <div style="position:relative;flex:1;">
                    <i class="fa fa-user" style="position:absolute;left:12px;top:50%;transform:translateY(-50%);color:#94a3b8;font-size:12px;pointer-events:none;"></i>
                    <input type="text" class="lk-pic-search-inp" id="pic-filter-inp"
                           placeholder="Filter nama PIC…" autocomplete="off"
                           oninput="filterPicGrid(this.value)">
                </div>
                <div style="font-size:11px;color:#94a3b8;font-weight:600;" id="pic-grid-count"></div>
            </div>

            <!-- PIC Loading -->
            <div id="pic-grid-loading" style="text-align:center;padding:20px;display:none;">
                <div class="lk-spin" style="width:20px;height:20px;border-width:2.5px;margin-bottom:6px;"></div>
                <div style="font-size:11px;color:#94a3b8;">Memuat daftar PIC…</div>
            </div>

            <!-- PIC Grid -->
            <div class="lk-pic-grid" id="pic-grid"></div>
        </div>
    ...
    <div id="lk-start">
        <div class="lk-state-panel">
            ...
            <div class="lk-state-sub">Ketik minimal 2 karakter di kolom pencarian di atas untuk melacak posisi dan riwayat perpindahan aset IT,<br>atau gunakan tab <strong>Berdasarkan PIC</strong> untuk melihat semua aset milik seseorang.</div>
        </div>
    </div>
    ...
    <div class="lk-body" id="lk-body" style="display:none;">
        ...
        <div class="lk-list-card">
            <div class="lk-list-head">
                <span class="lk-list-head-t" id="list-head-title">
                    <i class="fa fa-list"></i> Hasil Pencarian
                </span>
                <span class="lk-list-count" id="hasil-count">0</span>
            </div>
            <!-- PIC summary (muncul saat mode PIC) -->
            <div class="lk-pic-summary" id="pic-summary">
                <div class="lk-pic-sum-ava" id="psum-ava"></div>
                <div>
                    <div class="lk-pic-sum-name" id="psum-name"></div>
                    <div class="lk-pic-sum-sub" id="psum-sub"></div>
                </div>
                <div class="lk-pic-sum-badge" id="psum-badge"></div>
            </div>
...
let _tmr = null, _aid = null, _mode = 'aset';
let _picData = [], _selPic = null;
...
function switchMode(m) {
    _mode = m;
    document.getElementById('tab-aset').classList.toggle('active', m === 'aset');
    document.getElementById('tab-pic').classList.toggle('active',  m === 'pic');
    document.getElementById('panel-aset').style.display = m === 'aset' ? '' : 'none';
    document.getElementById('panel-pic').classList.toggle('show', m === 'pic');

    if (m === 'pic' && _picData.length === 0) loadPicList();
    if (m === 'aset') clearSearch();
}

/* ══════════════════════════════════════════
   PIC LIST
══════════════════════════════════════════ */
var PIC_COLORS = ['#6366f1','#10b981','#f59e0b','#ef4444','#3b82f6','#8b5cf6','#ec4899','#14b8a6','#f97316','#84cc16'];

function loadPicList() {
    document.getElementById('pic-grid-loading').style.display = 'block';
    document.getElementById('pic-grid').innerHTML = '';
    fetch(APP_URL + '/pages/lacak_aset_it.php?pic_list=1')
        .then(r => r.json())
        .then(data => {
            _picData = data;
            document.getElementById('pic-grid-loading').style.display = 'none';
            renderPicGrid(data);
        });
}

function renderPicGrid(data) {
    var grid = document.getElementById('pic-grid');
    var count = document.getElementById('pic-grid-count');
    count.textContent = data.length + ' PIC';
    grid.innerHTML = '';
    if (!data.length) {
        grid.innerHTML = '<div style="grid-column:1/-1;padding:20px;text-align:center;font-size:12px;color:#94a3b8;">Tidak ada PIC ditemukan</div>';
        return;
    }
    data.forEach(function(p, i) {
        var clr = PIC_COLORS[i % PIC_COLORS.length];
        var inisial = (p.nama||'?').split(' ').slice(0,2).map(function(w){return w[0];}).join('').toUpperCase();
        var el = document.createElement('div');
        el.className = 'lk-pic-card' + (_selPic && _selPic.id == p.id ? ' selected' : '');
        el.dataset.id = p.id;
        el.innerHTML =
            '<div class="lk-pic-ava" style="background:' + clr + ';">' + s(inisial) + '</div>' +
            '<div style="flex:1;min-width:0;">'+
                '<div class="lk-pic-name">' + s(p.nama) + '</div>' +
                '<div class="lk-pic-divisi">' + s(p.divisi || '—') + '</div>' +
            '</div>' +
            '<span class="lk-pic-badge">' + p.jumlah_aset + '</span>';
        el.onclick = function() { selectPIC(p, clr, inisial); };
        grid.appendChild(el);
    });
}

function filterPicGrid(q) {
    var filtered = !q ? _picData : _picData.filter(function(p) {
        return (p.nama||'').toLowerCase().includes(q.toLowerCase()) ||
               (p.divisi||'').toLowerCase().includes(q.toLowerCase());
    });
    renderPicGrid(filtered);
}

function selectPIC(pic, clr, inisial) {
    _selPic = pic;
    // Update selected bar
    var bar = document.getElementById('selected-pic-bar');
    bar.classList.add('show');
    document.getElementById('spic-ava').style.background = clr;
    document.getElementById('spic-ava').textContent = inisial;
    document.getElementById('spic-name').textContent = pic.nama;
    document.getElementById('spic-sub').textContent = (pic.divisi || '—') + ' · ' + pic.jumlah_aset + ' aset';
    document.getElementById('pic-search-row').style.display = 'none';
    document.getElementById('pic-grid').style.display = 'none';

    // Update list header
    document.getElementById('list-head-title').innerHTML = '<i class="fa fa-user"></i> Aset ' + s(pic.nama);
    // PIC summary di list
    var psum = document.getElementById('pic-summary');
    psum.classList.add('show');
    document.getElementById('psum-ava').style.background = clr;
    document.getElementById('psum-ava').textContent = inisial;
    document.getElementById('psum-name').textContent = pic.nama;
    document.getElementById('psum-sub').textContent = pic.divisi || '—';
    document.getElementById('psum-badge').textContent = pic.jumlah_aset + ' aset';
    ...
    document.getElementById('aset-list').innerHTML =
        '<div style="padding:14px;text-align:center;"><div class="lk-spin"></div></div>';
    document.getElementById('hasil-count').textContent = '…';

    fetch(APP_URL + '/pages/lacak_aset_it.php?search=1&pic=' + pic.id)
        .then(r => r.json())
        .then(function(data) {
...
function clearPIC() {
    _selPic = null;
    document.getElementById('selected-pic-bar').classList.remove('show');
    document.getElementById('pic-search-row').style.display = 'flex';
    document.getElementById('pic-grid').style.display = 'grid';
    document.getElementById('pic-filter-inp').value = '';
    renderPicGrid(_picData);
    document.getElementById('pic-summary').classList.remove('show');
    document.getElementById('lk-start').style.display = 'block';
    document.getElementById('lk-body').style.display = 'none';
}
...
function quickSearch(w) {
    inp.value = w;
    document.getElementById('lk-clear').style.display = 'flex';
    doSearch(w);
}
function doSearch(q) {
    document.getElementById('list-head-title').innerHTML = '<i class="fa fa-list"></i> Hasil Pencarian';
    document.getElementById('pic-summary').classList.remove('show');
    fetch(APP_URL + '/pages/lacak_aset_it.php?search=1&q=' + encodeURIComponent(q))
        .then(r => r.json()).then(data => { renderDd(data); renderList(data, false); });
}
function clearSearch() {
    inp.value = '';
    document.getElementById('lk-clear').style.display = 'none';
    closeDd();
    document.getElementById('lk-start').style.display = 'block';
    document.getElementById('lk-body').style.display = 'none';
    _aid = null;
...
function renderDd(data) {
    ...
    data.slice(0, 8).forEach(function(a) {
        ...
        el.innerHTML =
            '<div class="lk-dd-ico"><i class="fa fa-box"></i></div>' +
    ...
    if (data.length > 8) {
        ...
        m.style.cssText = 'padding:7px 13px;font-size:10.5px;color:#94a3b8;text-align:center;border-top:1px solid #f1f5f9;font-weight:600;';
        m.textContent = '+ ' + (data.length - 8) + ' lainnya';
        dd.appendChild(m);
    }
...
/* List */
function renderList(data, isPicMode) {
    document.getElementById('lk-start').style.display = 'none';
    document.getElementById('lk-body').style.display = 'grid';
    document.getElementById('hasil-count').textContent = data.length;
    ...
    if (!data.length) {
        list.innerHTML = '<div class="lk-no-result"><i class="fa fa-box-open"></i>' +
            (isPicMode ? 'PIC ini belum memiliki aset' : 'Tidak ada aset ditemukan') + '</div>';
        return;
    }
    ...
    data.forEach(function(a, i) {
        ...
        el.innerHTML =
            '<div class="lk-item-ico" style="background:'+c[0]+';"><i class="fa fa-box" style="color:'+c[1]+'"></i></div>' +
            ...
            '<div class="lk-item-meta">' +
                '<i class="fa fa-building" style="font-size:9px;"></i> ' + s(a.bagian_nama||'—') +
                // Tampilkan PIC di list item hanya saat mode keyword (bukan mode PIC)
                (!isPicMode && a.pj_nama ? ' &nbsp;·&nbsp; <i class="fa fa-user" style="font-size:9px;"></i> ' + s(a.pj_nama) : '') +
            '</div></div>' +
...
function renderDetail(a, mutasi, tiket) {
    ...
    var gw = '';
    if (a.garansi_sampai) {
        var gd = new Date(a.garansi_sampai), now = new Date();
...
function renderTL(aset, mutasi) {
    ...
    var bstyle={awal:'background:#eef2ff;color:#6366f1;',mutasi:'background:#f0fdf4;color:#15803d;',kini:'background:#fef9c3;color:#a16207;',batal:'background:#fee2e2;color:#b91c1c;'};
    ...
    items.forEach(function(item, idx) {
        ...
        if (t === 'mutasi' && !item.batal) {
            html += '<div class="lk-tc-route">' +
                ...
                '</div><div class="lk-tc-r-arr"><i class="fa fa-arrow-right"></i></div>' +
            ...
            if (item.kondisi_sesudah) {
                var kc2 = kClr[item.kondisi_sesudah]||'#94a3b8', kr = hexR(kc2);
