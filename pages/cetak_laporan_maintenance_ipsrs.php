...
// ── Parameter ────────────────────────────────────────────────────────────────
$mode       = $_GET['mode']      ?? 'semua';   // 'semua' | 'jenis'
$fjenis     = $_GET['jenis']     ?? '';
$fstatus    = $_GET['status']    ?? '';
...
// Stats jenis aset IPSRS (Medis vs Non-Medis)
$stat_jenis_aset = [];
foreach ($mnts as $m) {
    $ja = $m['jenis_aset'] ?? 'Non-Medis';
    $stat_jenis_aset[$ja] = ($stat_jenis_aset[$ja] ?? 0) + 1;
}

// Kelompokkan per jenis maintenance
$by_jenis = [];
foreach ($mnts as $m) {
...
$no_dok = 'RPT-MNT-IPSRS-' . date('Ymd') . '-' . strtoupper(substr(md5($mode.$fjenis.$fstatus.$tgl_dari.$tgl_sampai), 0, 4));

// ── Helpers ───────────────────────────────────────────────────────────────────
function ipsrsMntStatusStyle(string $s): array {
    return match ($s) {
...
function ipsrsMntKondisiStyle(string $k): array {
    return match ($k) {
...
function ipsrsMntJenisAsetStyle(string $j): array {
    return $j === 'Medis'
        ? ['bg' => '#fce7f3', 'fg' => '#9d174d']
        : ['bg' => '#eff6ff', 'fg' => '#1e40af'];
}
...
/* ── TABEL DATA ── */
.jenis-header { background: linear-gradient(135deg,#7c2d12,#c2410c); color: #fff; padding: 6px 10px; font-size: 9pt; font-weight: bold; border-radius: 3px 3px 0 0; margin-top: 14px; display: table; width: 100%; }
...
.data-tbl tbody tr.row-terlambat td { background: #fff1f2 !important; border-left: 3px solid #ef4444; }
.data-tbl tbody tr.row-segera td    { background: #fffbeb !important; border-left: 3px solid #f59e0b; }

/* ── Badges ── */
.status-badge  { display: inline-block; padding: 2px 7px; border-radius: 3px; font-size: 7.5pt; font-weight: bold; white-space: nowrap; }
...
.no-mnt-code   { font-family: 'Courier New', monospace; font-size: 7.5pt; font-weight: bold; background: #fff7ed; color: #c2410c; border: 1px solid #fed7aa; padding: 1px 5px; border-radius: 3px; white-space: nowrap; }
...
/* ── ALERT ── */
.alert-box { display: table; width: 100%; margin-bottom: 10px; padding: 8px 12px; border-radius: 4px; font-size: 8.5pt; }
...
<div class="kop">
  ...
  <div class="kop-text">
    ...
    <div class="kop-sub">IPSRS &mdash; Instalasi Pemeliharaan Sarana Rumah Sakit &mdash; Sistem Manajemen Maintenance</div>
  </div>
  <div class="kop-right">
...
<div class="report-title-block">
  ...
  <div class="report-main-title">Laporan Maintenance IPSRS</div>
...
<table class="doc-info">
  ...
  <tr>
    ...
    <td class="di-val">Catatan Maintenance Aset IPSRS</td>
  ...
  <tr>
    ...
    <td class="di-val">
      <?= ($tgl_dari !== '' || $tgl_sampai !== '')
        ? (($tgl_dari   !== '' ? date('d/m/Y', strtotime($tgl_dari))   : '—') . ' s/d ' .
           ($tgl_sampai !== '' ? date('d/m/Y', strtotime($tgl_sampai)) : 'Sekarang'))
        : 'Semua Periode' ?>
    </td>
    <td class="di-label">Total Catatan</td>
    <td class="di-val">
      <strong><?= $total_mnt ?> catatan</strong>
      &nbsp;|&nbsp;
      <span style="color:#9d174d;font-weight:bold;">Medis: <?= $stat_jenis_aset['Medis'] ?? 0 ?></span>
      &nbsp;&middot;&nbsp;
      <span style="color:#1e40af;font-weight:bold;">Non-Medis: <?= $stat_jenis_aset['Non-Medis'] ?? 0 ?></span>
    </td>
  </tr>
</table>
...
<div class="exec-box">
  Laporan ini menyajikan data catatan maintenance IPSRS
  <?= ($mode === 'jenis' && $fjenis !== '') ? 'untuk jenis <strong>' . htmlspecialchars($fjenis) . '</strong>' : 'untuk <strong>semua jenis maintenance</strong>' ?>.
  Total catatan yang tercatat sebanyak <strong><?= $total_mnt ?> catatan</strong> dalam <strong><?= count($by_jenis) ?> jenis maintenance</strong>,
  mencakup <strong><?= $stat_jenis_aset['Medis'] ?? 0 ?> catatan aset Medis</strong>
  dan <strong><?= $stat_jenis_aset['Non-Medis'] ?? 0 ?> catatan aset Non-Medis</strong>.
  Dari total tersebut, <strong><?= $jml_selesai ?> catatan (<?= $pct_selesai ?>%)</strong> telah selesai,
  dan <strong><?= $jml_proses ?> catatan</strong> masih dalam proses.
  <?php if ($jml_terlambat > 0): ?>
  <strong style="color:#ef4444;">Terdapat <?= $jml_terlambat ?> aset yang jadwal maintenance-nya telah terlambat — perlu tindakan segera.</strong>
  <?php endif; ?>
  Total biaya maintenance yang tercatat: <strong>Rp <?= number_format($total_biaya, 0, ',', '.') ?></strong>.
  Siklus maintenance IPSRS ditetapkan setiap 3 bulan sekali.
</div>
...
<div class="sec">
  ...
  <div class="sec-title"><span class="sec-title-text">Statistik Maintenance berdasarkan Status</span></div>
</div>
  ...
  <tr>
    <td class="kpi-td">
      <div class="kpi-card" style="border-top-color:#ea580c;">
        ...
        <div class="k-val" style="color:#ea580c;"><?= $total_mnt ?></div>
        <div class="k-desc">Seluruh catatan maintenance IPSRS</div>
      </div>
    </td>
    <td class="kpi-td">
      <div class="kpi-card" style="border-top-color:#ec4899;">
        <div class="k-label">Aset Medis</div>
        <div class="k-val" style="color:#ec4899;"><?= $stat_jenis_aset['Medis'] ?? 0 ?></div>
        <div class="k-desc">Maintenance peralatan medis</div>
      </div>
    </td>
    <td class="kpi-td">
      <div class="kpi-card" style="border-top-color:#3b82f6;">
        <div class="k-label">Aset Non-Medis</div>
        <div class="k-val" style="color:#3b82f6;"><?= $stat_jenis_aset['Non-Medis'] ?? 0 ?></div>
        <div class="k-desc">Maintenance sarana &amp; prasarana</div>
      </div>
    </td>
    <td class="kpi-td">
    ...
    <td class="kpi-td">
      <div class="kpi-card" style="border-top-color:#3b82f6;">
        ...
        <div class="k-desc">Sedang dalam pengerjaan</div>
      </div>
    </td>
    <td class="kpi-td">
    ...
    <td class="kpi-td">
      <div class="kpi-card" style="border-top-color:#0ea5e9;">
        ...
        <div class="k-desc">Akumulasi biaya tercatat</div>
      </div>
    </td>
  </tr>
</table>
...
<div class="sec">
  ...
  <div class="sec-title">
    ...
    <span class="sec-title-rule">Dikelompokkan per Jenis Maintenance</span>
  </div>
</div>
<table class="summ-tbl">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td style="text-align:right;font-size:7.5pt;"><?= $biaya_j > 0 ? 'Rp '.number_format($biaya_j,0,',','.') : '—' ?></td>
      <td>
...
<div class="sec">
  ...
  <div class="sec-title">
    <span class="sec-title-text">Detail Catatan Maintenance IPSRS</span>
    <span class="sec-title-rule">Diurutkan per Jenis &nbsp;|&nbsp; &#9632; Merah = Terlambat &nbsp; &#9632; Kuning = Segera (&lt;14 hari)</span>
  </div>
</div>
...
<?php else: foreach ($by_jenis as $jenis => $items):
  $biaya_subtotal = array_sum(array_column($items, 'biaya'));
?>

<div class="jenis-header">
  <span class="jenis-header-l">&#9654; <?= htmlspecialchars($jenis) ?></span>
  <span class="jenis-header-r">
    <?= count($items) ?> catatan &nbsp;|&nbsp;
    Selesai: <?= count(array_filter($items, fn($x) => $x['status']==='Selesai')) ?> &nbsp;
    Proses: <?= count(array_filter($items, fn($x) => $x['status']==='Dalam Proses')) ?> &nbsp;
    Medis: <?= count(array_filter($items, fn($x) => ($x['jenis_aset']??'')==='Medis')) ?> &nbsp;
    Non-Medis: <?= count(array_filter($items, fn($x) => ($x['jenis_aset']??'')==='Non-Medis')) ?>
  </span>
</div>
...
<table class="data-tbl">
  <thead>
    <tr>
      ...
      <th style="width:90px;">No. Maintenance</th>
      <th style="width:115px;">Aset IPSRS</th>
      <th style="width:52px;">Jenis Aset</th>
      <th style="width:80px;">Teknisi</th>
      <th style="width:58px;">Tgl Maint.</th>
      <th style="width:62px;">Kondisi Sbl</th>
      <th style="width:62px;">Kondisi Ssd</th>
      <th>Temuan / Tindakan</th>
      <th style="width:58px;">Status</th>
      <th style="width:68px;">Mnt. Berikut</th>
      <th style="width:68px;">Biaya (Rp)</th>
    </tr>
  </thead>
  <tbody>
    <?php $no_item = 0; foreach ($items as $m):
      $no_item++;
      $sp        = $m['status'] ?? '';
      $sstyle    = ipsrsMntStatusStyle($sp);
      $kstyle_sbl = ipsrsMntKondisiStyle($m['kondisi_sebelum'] ?? '');
      $kstyle_ssd = ipsrsMntKondisiStyle($m['kondisi_sesudah'] ?? '');
      $ja_style   = ipsrsMntJenisAsetStyle($m['jenis_aset'] ?? 'Non-Medis');
      ...
      if ($m['tgl_maintenance_berikut']) {
          ...
          if ($selisih_next < 0)       { $next_color = '#b91c1c'; $row_class = 'row-terlambat'; }
          elseif ($selisih_next <= 14) { $next_color = '#d97706'; $row_class = 'row-segera'; }
      }

      $aset_nama = $m['aset_nama_db'] ?: ($m['aset_nama'] ?? '—');
      $tek_nama  = $m['tek_nama_db']  ?: ($m['teknisi_nama'] ?? '—');
      $temuan    = $m['temuan']    ? mb_strimwidth($m['temuan'],    0, 48, '…') : '';
      $tindakan  = $m['tindakan']  ? mb_strimwidth($m['tindakan'],  0, 48, '…') : '';
    ?>
    <tr class="<?= $row_class ?>">
      <td style="text-align:center;color:#94a3b8;"><?= $no_item ?></td>
      <td><span class="no-mnt-code"><?= htmlspecialchars($m['no_maintenance'] ?? '—') ?></span></td>
      <td>
        <div style="font-weight:bold;font-size:7.8pt;color:#1e293b;"><?= htmlspecialchars($aset_nama) ?></div>
        <div style="font-size:6.5pt;color:#94a3b8;font-family:'Courier New',monospace;"><?= htmlspecialchars($m['no_inventaris'] ?? '') ?></div>
        <?php if ($m['aset_kat']): ?>
          <div style="font-size:6.5pt;color:#94a3b8;"><?= htmlspecialchars($m['aset_kat']) ?></div>
        <?php endif; ?>
      </td>
      <td>
        <?php $ja = $m['jenis_aset'] ?? 'Non-Medis'; ?>
        <span class="jenis-badge" style="background:<?= $ja_style['bg'] ?>;color:<?= $ja_style['fg'] ?>;">
          <?= $ja === 'Medis' ? 'Medis' : 'Non-Med.' ?>
        </span>
      </td>
      <td>
        <div style="font-size:7.8pt;"><?= htmlspecialchars($tek_nama) ?></div>
        <?php if ($m['tek_divisi']): ?>
          <div style="font-size:6.5pt;color:#94a3b8;"><?= htmlspecialchars($m['tek_divisi']) ?></div>
        <?php endif; ?>
      </td>
      ...
      <td>
        <?php if ($m['kondisi_sebelum']): ?>
          <span class="kondisi-badge" style="background:<?= $kstyle_sbl['bg'] ?>;color:<?= $kstyle_sbl['fg'] ?>;"><?= htmlspecialchars($m['kondisi_sebelum']) ?></span>
        <?php else: ?><span style="color:#cbd5e1;font-size:7pt;">—</span><?php endif; ?>
      </td>
      <td>
        <?php if ($m['kondisi_sesudah']): ?>
          <span class="kondisi-badge" style="background:<?= $kstyle_ssd['bg'] ?>;color:<?= $kstyle_ssd['fg'] ?>;"><?= htmlspecialchars($m['kondisi_sesudah']) ?></span>
        <?php else: ?><span style="color:#cbd5e1;font-size:7pt;">—</span><?php endif; ?>
      </td>
      <td style="font-size:7.5pt;">
        <?php if ($temuan): ?>
          <div><span style="font-weight:bold;color:#475569;">Temuan:</span> <?= htmlspecialchars($temuan) ?></div>
        <?php endif; ?>
        <?php if ($tindakan): ?>
          <div style="margin-top:2px;"><span style="font-weight:bold;color:#475569;">Tindakan:</span> <?= htmlspecialchars($tindakan) ?></div>
        <?php endif; ?>
        <?php if (!$temuan && !$tindakan): ?><span style="color:#cbd5e1;">—</span><?php endif; ?>
        <?php if ($m['keterangan']): ?>
          <div style="margin-top:2px;font-size:6.5pt;color:#94a3b8;font-style:italic;"><?= htmlspecialchars(mb_strimwidth($m['keterangan'],0,40,'…')) ?></div>
        <?php endif; ?>
      </td>
      <td>
      ...
      <td style="font-size:7.5pt;white-space:nowrap;">
        <?php if ($m['tgl_maintenance_berikut']): ?>
          <div style="font-weight:bold;color:<?= $next_color ?: '#1e293b' ?>;"><?= date('d/m/Y', strtotime($m['tgl_maintenance_berikut'])) ?></div>
          <?php if ($selisih_next !== null): ?>
            <div style="font-size:7pt;color:<?= $next_color ?: '#64748b' ?>;">
      ...
      <td style="font-size:7.5pt;text-align:right;color:#0f172a;">
        <?= $m['biaya'] ? number_format((int)$m['biaya'], 0, ',', '.') : '—' ?>
      </td>
    </tr>
    <?php endforeach; ?>
  </tbody>
  <tfoot>
    <tr>
      ...
      <td colspan="2" style="text-align:right;">
        <?= $biaya_subtotal > 0 ? 'Rp '.number_format($biaya_subtotal, 0, ',', '.') : '—' ?>
      </td>
    </tr>
  </tfoot>
</table>
...
<div style="margin-top:14px;padding:10px 14px;background:#fff7ed;border:1px solid #fed7aa;border-radius:4px;display:table;width:100%;">
  <div style="display:table-cell;font-weight:bold;color:#7c2d12;font-size:9pt;">
    Total Biaya Maintenance IPSRS<?= ($mode === 'jenis' && $fjenis !== '') ? ' — Jenis: '.htmlspecialchars($fjenis) : '' ?>
  </div>
  <div style="display:table-cell;text-align:right;font-size:12pt;font-weight:bold;color:#ea580c;">
...
<table style="width:100%;border-collapse:collapse;margin-top:10px;border:1px solid #fed7aa;font-size:7.5pt;">
  <tr>
    <td style="padding:5px 9px;background:#fff7ed;font-weight:bold;color:#92400e;border-right:1px solid #fed7aa;width:13%;">Keterangan Status</td>
    <?php foreach ([
      ['Selesai','#dcfce7','#15803d'],['Dalam Proses','#dbeafe','#1e40af'],
      ['Ditunda','#fef9c3','#a16207'],['Dibatalkan','#fee2e2','#b91c1c'],
    ] as [$lbl,$bg,$fg]): ?>
    <td style="padding:5px 9px;border-right:1px solid #fed7aa;">
      <span class="status-badge" style="background:<?= $bg ?>;color:<?= $fg ?>;"><?= $lbl ?></span>
    </td>
    <?php endforeach; ?>
    <td style="padding:5px 9px;border-right:1px solid #fed7aa;background:#fff1f2;">
      <span style="color:#b91c1c;font-weight:bold;">&#9632; Merah kiri</span> = Terlambat
    </td>
    <td style="padding:5px 9px;background:#fffbeb;">
      <span style="color:#d97706;font-weight:bold;">&#9632; Kuning kiri</span> = Segera (&lt;14 hari)
    </td>
  </tr>
</table>
...
<div class="page-footer">
  <div class="pf-left">
    FixSmart Helpdesk &mdash; Laporan Maintenance IPSRS &mdash; Dicetak: <?= date('d/m/Y H:i:s') ?> WIB<br>
    Dokumen ini bersifat rahasia dan hanya untuk keperluan internal manajemen IPSRS / RS.
  </div>
  <div class="pf-right">
    No. Dok: <?= $no_dok ?> &nbsp;|&nbsp; Total: <?= $total_mnt ?> catatan<br>
...
$sp_slug    = $fstatus !== '' ? '_' . preg_replace('/[^a-zA-Z0-9]/', '_', $fstatus) : '';
$filename   = 'Laporan_Maintenance_IPSRS' . $jenis_slug . $sp_slug . '_' . date('Ymd_His') . '.pdf';
