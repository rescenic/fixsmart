...
// Tambah kolom kota_domisili & tgl_terbit_str/sip jika belum ada (untuk upgrade DB lama)
$alter_cols = [
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'simpan_sdm') {
        ...
        if (!$uid) { setFlash('danger', 'User ID tidak valid.'); redirect(APP_URL.'/pages/master_karyawan.php'); }

        $d = [];
        $fields_text = ['nik_ktp','nik_rs','gelar_depan','gelar_belakang','tempat_lahir','kota_ktp','provinsi_ktp',
                        'kode_pos_ktp','jurusan','universitas','jenis_tenaga','no_bpjs_kes','no_bpjs_tk','no_npwp',
                        'no_rekening','bank','atas_nama_rek','no_str','no_sip','no_sik','spesialisasi','catatan',
                        'no_hp','no_hp_darurat','kontak_darurat','hubungan_darurat','kota_domisili'];
        $fields_text2 = ['alamat_ktp','alamat_domisili','kompetensi'];
        $fields_enum  = ['jenis_kelamin','golongan_darah','agama','status_pernikahan','pendidikan_terakhir',
                         'jenis_karyawan','status_kepegawaian'];
        $fields_date  = ['tgl_lahir','tgl_masuk','tgl_kontrak_mulai','tgl_kontrak_selesai','tgl_pengangkatan',
                         'tgl_terbit_str','tgl_exp_str','tgl_terbit_sip','tgl_exp_sip','tgl_exp_sik'];
        ...
        // Cek ada record belum
        $ex = $pdo->prepare("SELECT id FROM sdm_karyawan WHERE user_id=?"); $ex->execute([$uid]);
        if ($ex->fetchColumn()) {
    ...
    if ($act === 'hapus') {
        ...
        if ($id && $id !== (int)($_SESSION['user_id'] ?? 0)) {
            $r = $pdo->prepare("SELECT nama FROM users WHERE id=?"); $r->execute([$id]);
            $nm = $r->fetchColumn();
            if ($nm) {
        ...
        } else { setFlash('warning', 'Tidak dapat menghapus akun sendiri.'); }
        redirect(APP_URL.'/pages/master_karyawan.php');
    }
...
$cnt=$pdo->prepare("SELECT COUNT(*) FROM users u $wsql");$cnt->execute($params);
...
$stm=$pdo->prepare("
    SELECT u.*, s.id AS sdm_id,
           s.nik_rs, s.jenis_karyawan, s.status_kepegawaian, s.tgl_masuk, s.no_str,
           s.tgl_exp_str, s.tgl_exp_sip, s.gelar_depan, s.gelar_belakang
    FROM users u
    LEFT JOIN sdm_karyawan s ON s.user_id = u.id
    $wsql ORDER BY u.nama ASC LIMIT $per_page OFFSET $offset
");
$stm->execute($params);
$list=$stm->fetchAll(PDO::FETCH_ASSOC);

$bagian_list=[];
try{$bagian_list=$pdo->query("SELECT nama FROM bagian WHERE status='aktif' ORDER BY urutan,nama")->fetchAll(PDO::FETCH_COLUMN);}catch(Exception $e){}
...
try{
    $s=$pdo->query("SELECT COUNT(*) total, SUM(u.status='aktif') aktif, SUM(u.status='nonaktif') nonaktif,
        SUM(role='admin') admin_cnt, SUM(role='teknisi') teknisi_cnt, SUM(role='teknisi_ipsrs') ipsrs_cnt,
        SUM(role='hrd') hrd_cnt, SUM(role='user') user_cnt
        FROM users u")->fetch(PDO::FETCH_ASSOC);
    if($s)$stats=array_merge($stats,$s);
    $sdm_cnt=$pdo->query("SELECT COUNT(*) FROM sdm_karyawan WHERE tgl_masuk IS NOT NULL AND jenis_karyawan IS NOT NULL")->fetchColumn();
...
<div class="content">
  ...
  <?php if(!$_hrd_in_enum):?>
  <div style="background:#fffbeb;border:1px solid #fde68a;border-radius:8px;padding:11px 14px;margin-bottom:14px;font-size:12.5px;color:#92400e;display:flex;align-items:flex-start;gap:8px;">
    ...
    <code style="display:block;margin-top:4px;background:#fef3c7;padding:5px 10px;border-radius:4px;font-size:11.5px;">
      ...
      <div class="panel-bd">
        <div style="display:flex;flex-wrap:wrap;gap:8px;align-items:flex-end;">
          <div>
            ...
            <input type="text" name="nama" class="form-control" style="width:180px;height:34px;" placeholder="Ketik…" value="<?=htmlspecialchars($f_nama)?>">
          </div>
          <div>
            ...
            <select name="divisi" class="form-control" style="min-width:130px;height:34px;">
              ...
              <option value="<?=htmlspecialchars($b)?>" <?=$f_divisi===$b?'selected':''?>><?=htmlspecialchars($b)?></option>
              <?php endforeach;?>
            </select>
          </div>
          <div>
          ...
          <div style="display:flex;gap:6px;">
            ...
            <a href="master_karyawan.php" class="btn btn-default" style="height:34px;display:inline-flex;align-items:center;gap:5px;"><i class="fa fa-rotate-left"></i> Reset</a>
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="panel">
    <div class="panel-hd">
      <h5>
        ...
        <span style="font-size:11px;background:#f0fdf4;color:#16a34a;border:1px solid #bbf7d0;padding:1px 8px;border-radius:10px;font-weight:600;margin-left:6px;">filter aktif</span>
        <?php endif;?>
      </h5>
    </div>
    <div class="panel-bd np tbl-wrap">
      <table>
        ...
        <tbody>
        <?php if(empty($list)):?>
        <tr><td colspan="8" class="td-empty"><i class="fa fa-users"></i> Tidak ada data karyawan</td></tr>
        <?php else: foreach($list as $i=>$k):
          ...
          $nama_tampil = ($k['gelar_depan'] ? $k['gelar_depan'].' ' : '') . htmlspecialchars($k['nama']) . ($k['gelar_belakang'] ? ', '.$k['gelar_belakang'] : '');
          $sdm_ada = !empty($k['sdm_id']);
          $exp_warning = '';
          if($k['tgl_exp_str'] && strtotime($k['tgl_exp_str']) < strtotime('+30 days')) $exp_warning = 'STR';
          if($k['tgl_exp_sip'] && strtotime($k['tgl_exp_sip']) < strtotime('+30 days')) $exp_warning .= ($exp_warning?'/':'').'SIP';
          // Escape nama untuk JS — pakai json_encode agar aman
          $nama_js = json_encode($k['nama']);
        ?>
        <tr>
          <td style="color:#bbb;font-size:12px;"><?=$offset+$i+1?></td>
          <td>
            <div style="display:flex;align-items:center;gap:8px;">
              <div style="width:34px;height:34px;border-radius:50%;background:linear-gradient(135deg,#00e5b0,#00c896);color:#0a0f14;font-size:11px;font-weight:800;display:inline-flex;align-items:center;justify-content:center;flex-shrink:0;">
              ...
              <div>
                ...
                <div style="font-size:10.5px;color:#9ca3af;">
                  ...
                  <?php if($is_self):?>
                  &nbsp;<span style="background:#f0fdf4;color:#16a34a;border:1px solid #bbf7d0;padding:0 5px;border-radius:8px;font-size:9.5px;font-weight:700;">Anda</span>
                  <?php endif;?>
                </div>
              </div>
            </div>
          ...
          <td style="font-size:12px;">
            ...
            <?php if($k['no_hp']??''):?>
            <br><span style="font-size:11px;color:#888;"><i class="fa fa-phone" style="font-size:10px;"></i> <?=htmlspecialchars($k['no_hp']??'')?></span>
            <?php endif;?>
          </td>
          <td style="font-size:12px;">
          ...
          <td>
            <span style="font-size:11px;font-weight:700;padding:2px 9px;border-radius:20px;<?=$rs_style[$k['role']]??'background:#f3f4f6;color:#374151;'?>">
          ...
          <td>
            <?php if($sdm_ada):?>
              ...
              <?php if($exp_warning):?>
              <br><span style="font-size:9.5px;color:#dc2626;font-weight:700;"><i class="fa fa-triangle-exclamation"></i> Exp. <?=$exp_warning?></span>
              <?php endif;?>
            <?php else:?>
          ...
          <td>
            <span style="font-size:11px;font-weight:700;padding:2px 8px;border-radius:10px;background:<?=$k['status']==='aktif'?'#d1fae5':'#f3f4f6'?>;color:<?=$k['status']==='aktif'?'#065f46':'#6b7280'?>;">
              <?=$k['status']==='aktif'?'● Aktif':'○ Nonaktif'?>
            </span>
          </td>
          <td style="white-space:nowrap;">
            <!-- Tombol SDM — pakai data-* attr, bukan inline onclick langsung -->
            <button
              type="button"
            ...
            <!-- Hapus -->
            <?php if(!$is_self):?>
            <form method="POST" style="display:inline;" onsubmit="return confirm('Hapus akun <?=addslashes(htmlspecialchars($k['nama']))?>?')">
              <input type="hidden" name="action" value="hapus">
              <input type="hidden" name="id" value="<?=(int)$k['id']?>">
              <button type="submit" class="btn btn-danger btn-sm" title="Hapus"><i class="fa fa-trash"></i></button>
            </form>
            <?php else:?>
            <button class="btn btn-sm btn-default" disabled title="Tidak dapat menghapus akun sendiri"><i class="fa fa-lock" style="color:#bbb;"></i></button>
            <?php endif;?>
          </td>
        </tr>
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;border-top:1px solid #e5e7eb;flex-wrap:wrap;gap:8px;">
      <div style="font-size:11.5px;color:#6b7280;">Halaman <?=$page_cur?> dari <?=$total_pages?> &mdash; <?=number_format($total_rows)?> data</div>
      <div style="display:flex;gap:4px;">
        ...
        <a href="?p=<?=$pg?><?=$qs?>" class="btn btn-sm <?=$pg===$page_cur?'btn-primary':'btn-default'?>"><?=$pg?></a>
        <?php endfor;?>
  ...
  <div class="sdm-modal">
    ...
    <div class="sdm-info-bar">
      ...
      <div>
        ...
        <div class="sdm-info-sub">Melengkapi data SDM karyawan</div>
      </div>
    ...
    <!-- Tabs -->
    <div class="sdm-tabs">
      <button type="button" class="sdm-tab active" data-pane="t-identitas"><i class="fa fa-id-card"></i> Identitas</button>
      <button type="button" class="sdm-tab" data-pane="t-kontak"><i class="fa fa-location-dot"></i> Kontak &amp; Alamat</button>
      <button type="button" class="sdm-tab" data-pane="t-kepegawaian"><i class="fa fa-briefcase"></i> Kepegawaian</button>
      <button type="button" class="sdm-tab" data-pane="t-keuangan"><i class="fa fa-credit-card"></i> Keuangan</button>
      <button type="button" class="sdm-tab" data-pane="t-lisensi"><i class="fa fa-stethoscope"></i> Lisensi</button>
    </div>
    ...
    <form method="POST" id="sdm-form">
      ...
      <div class="sdm-mbody">
        ...
        <div class="sdm-tab-pane active" id="t-identitas">
          ...
          <div class="sdm-g3">
            <div class="sdm-fg">
              <label>NIK RS <span class="sdm-req">*</span></label>
              <input type="text" name="nik_rs" id="s-nik_rs" placeholder="Nomor Induk Karyawan RS">
            </div>
            <div class="sdm-fg">
          ...
          <div class="sdm-g3">
            ...
            <div class="sdm-fg">
              <label>Gelar Belakang <span class="opt">(M.Kes, Sp.A, dll)</span></label>
          ...
          <div class="sdm-g4">
            ...
            <div class="sdm-fg">
              ...
              <input type="text" name="jurusan" id="s-jurusan" placeholder="Keperawatan / Kedokteran…">
            </div>
            <div class="sdm-fg">
              <label>Universitas / Institusi</label>
              <input type="text" name="universitas" id="s-universitas" placeholder="Nama universitas">
            </div>
            <div class="sdm-fg">
        ...
        <div class="sdm-tab-pane" id="t-kontak">
          ...
          <div class="sdm-g3">
            <div class="sdm-fg">
              <label>No. HP / WhatsApp <span class="sdm-req">*</span></label>
            ...
            <div class="sdm-fg">
              ...
              <input type="text" name="kontak_darurat" id="s-kontak_darurat" placeholder="Nama keluarga / kerabat">
            </div>
          </div>
          <div class="sdm-g3">
            <div class="sdm-fg">
              <label>Hubungan Kontak Darurat</label>
              <input type="text" name="hubungan_darurat" id="s-hubungan_darurat" placeholder="Istri / Suami / Orang Tua…">
            </div>
          </div>

          <div class="sdm-sec"><i class="fa fa-location-dot"></i> Alamat Sesuai KTP</div>
          <div class="sdm-fg">
          ...
          <div class="sdm-g4">
            <div class="sdm-fg">
              ...
              <input type="text" name="kota_ktp" id="s-kota_ktp">
            </div>
            <div class="sdm-fg">
              <label>Provinsi</label>
              <input type="text" name="provinsi_ktp" id="s-provinsi_ktp">
            </div>
            <div class="sdm-fg">
              <label>Kode Pos</label>
              <input type="text" name="kode_pos_ktp" id="s-kode_pos_ktp" maxlength="10">
            </div>
          </div>
          ...
          <div class="sdm-fg">
            ...
            <textarea name="alamat_domisili" id="s-alamat_domisili" placeholder="Kosongkan jika sama dengan alamat KTP…"></textarea>
          </div>
          <div class="sdm-g2">
            <div class="sdm-fg">
              <label>Kota Domisili</label>
              <input type="text" name="kota_domisili" id="s-kota_domisili">
            </div>
          </div>
        </div>
        ...
        <div class="sdm-tab-pane" id="t-kepegawaian">
          ...
          <div class="sdm-g3">
            ...
            <div class="sdm-fg">
              ...
              <input type="text" name="jenis_tenaga" id="s-jenis_tenaga" placeholder="Dokter Umum / Perawat / Bidan…">
            </div>
            <div class="sdm-fg">
              <label>Spesialisasi</label>
              <input type="text" name="spesialisasi" id="s-spesialisasi" placeholder="Sp.A / ICU / IGD / — (non-medis)">
            </div>
          </div>
          <div class="sdm-g3">
            <div class="sdm-fg">
              ...
              <select name="jabatan_id" id="s-jabatan_id">
                ...
                <option value="<?=(int)$jb['id']?>"><?=htmlspecialchars($jb['nama'])?><?=$jb['kode']?' ('.$jb['kode'].')':''?></option>
                <?php endforeach;?>
              </select>
            </div>
            <div class="sdm-fg">
          ...
          <div class="sdm-g4">
            <div class="sdm-fg">
              <label>Tanggal Masuk <span class="sdm-req">*</span></label>
              <input type="date" name="tgl_masuk" id="s-tgl_masuk">
            </div>
            <div class="sdm-fg">
              <label>Awal Kontrak</label>
              <input type="date" name="tgl_kontrak_mulai" id="s-tgl_kontrak_mulai">
            </div>
            <div class="sdm-fg">
              <label>Akhir Kontrak</label>
              <input type="date" name="tgl_kontrak_selesai" id="s-tgl_kontrak_selesai">
            </div>
            <div class="sdm-fg">
              <label>Tgl Pengangkatan Tetap</label>
              <input type="date" name="tgl_pengangkatan" id="s-tgl_pengangkatan">
            </div>
          </div>
          <div class="sdm-fg">
        ...
        <div class="sdm-tab-pane" id="t-keuangan">
          ...
          <div class="sdm-g3">
            <div class="sdm-fg">
              ...
              <input type="text" name="no_bpjs_kes" id="s-no_bpjs_kes" placeholder="0001xxxxxxxxx">
            </div>
            <div class="sdm-fg">
              <label>No. BPJS Ketenagakerjaan</label>
              <input type="text" name="no_bpjs_tk" id="s-no_bpjs_tk" placeholder="Nomor BPJS TK">
            </div>
            <div class="sdm-fg">
              <label>No. NPWP</label>
              <input type="text" name="no_npwp" id="s-no_npwp" placeholder="XX.XXX.XXX.X-XXX.XXX">
            </div>
          </div>
          ...
          <div class="sdm-g3">
            ...
            <div class="sdm-fg">
              ...
              <input type="text" name="no_rekening" id="s-no_rekening" placeholder="Nomor rekening">
            </div>
            <div class="sdm-fg">
              <label>Atas Nama</label>
              <input type="text" name="atas_nama_rek" id="s-atas_nama_rek" placeholder="Nama pemilik rekening">
            </div>
          </div>
        </div>
        ...
        <div class="sdm-tab-pane" id="t-lisensi">
          <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:10px 14px;margin-bottom:12px;">
            <p class="sdm-hint" style="margin:0;"><i class="fa fa-circle-info" style="color:#00c896;"></i> Untuk karyawan <strong>Non-Medis</strong>, isi dengan <code>-</code> atau kosongkan saja.</p>
          </div>
          ...
          <div class="sdm-g3">
            <div class="sdm-fg">
              ...
              <input type="text" name="no_str" id="s-no_str" placeholder="Nomor STR atau -">
            </div>
            <div class="sdm-fg">
              <label>Tgl Terbit STR</label>
              <input type="date" name="tgl_terbit_str" id="s-tgl_terbit_str">
            </div>
            <div class="sdm-fg">
              <label>Exp. STR</label>
              <input type="date" name="tgl_exp_str" id="s-tgl_exp_str">
            </div>
          </div>
          ...
          <div class="sdm-g3">
            <div class="sdm-fg">
              ...
              <input type="text" name="no_sip" id="s-no_sip" placeholder="Nomor SIP atau -">
            </div>
            <div class="sdm-fg">
              <label>Tgl Terbit SIP</label>
              <input type="date" name="tgl_terbit_sip" id="s-tgl_terbit_sip">
            </div>
            <div class="sdm-fg">
              <label>Exp. SIP</label>
              <input type="date" name="tgl_exp_sip" id="s-tgl_exp_sip">
            </div>
          </div>
          ...
          <div class="sdm-g3">
            <div class="sdm-fg">
              ...
              <input type="text" name="no_sik" id="s-no_sik" placeholder="Nomor SIK atau -">
            </div>
            <div class="sdm-fg">
              <label>Exp. SIK</label>
              <input type="date" name="tgl_exp_sik" id="s-tgl_exp_sik">
            </div>
          </div>
          ...
          <div class="sdm-fg">
            ...
            <textarea name="kompetensi" id="s-kompetensi" placeholder="BLS, ACLS, BTCLS, PONEK, dll. Pisahkan dengan koma."></textarea>
          </div>
        </div>
      ...
      <div class="sdm-mft">
        <button type="button" class="btn btn-default" id="sdm-cancel-btn"><i class="fa fa-times"></i> Batal</button>
        <button type="submit" class="btn btn-primary"><i class="fa fa-save"></i> Simpan Data SDM</button>
      </div>
    </form>
  </div><!-- /.sdm-modal -->
</div><!-- /#sdm-modal -->
...
(function () {
  ...
  document.querySelectorAll('.sdm-tab').forEach(function (btn) {
    btn.addEventListener('click', function () {
      switchTab(btn, btn.getAttribute('data-pane'));
    });
  });
  ...
  function fetchSdmData(uid) {
    ...
    fetch('ajax_sdm.php?user_id=' + uid, { credentials: 'same-origin' })
      ...
      .then(function (d) {
        ...
        if (!d || d.error) return; // belum ada data, form kosong saja

        var fields = [
          'nik_rs','nik_ktp','gelar_depan','gelar_belakang','tempat_lahir',
          'kota_ktp','provinsi_ktp','kode_pos_ktp','jurusan','universitas',
          'jenis_tenaga','no_bpjs_kes','no_bpjs_tk','no_npwp','no_rekening',
          'bank','atas_nama_rek','no_str','no_sip','no_sik','spesialisasi',
          'catatan','no_hp','no_hp_darurat','kontak_darurat','hubungan_darurat',
          'tgl_lahir','tgl_masuk','tgl_kontrak_mulai','tgl_kontrak_selesai',
          'tgl_pengangkatan','tgl_exp_str','tgl_terbit_str','tgl_exp_sip',
          'tgl_terbit_sip','tgl_exp_sik','jenis_kelamin','golongan_darah',
          'agama','status_pernikahan','pendidikan_terakhir','jenis_karyawan',
          'status_kepegawaian','jumlah_anak','tahun_lulus','alamat_ktp',
          'alamat_domisili','kompetensi','jabatan_id','kota_domisili'
        ];

        fields.forEach(function (f) {
          var el = document.getElementById('s-' + f);
          if (!el) return;
          el.value = (d[f] !== null && d[f] !== undefined) ? d[f] : '';
        });
      })
      .catch(function (err) {
        loading.style.display = 'none';
        console.warn('SDM fetch error:', err);
        // Modal tetap terbuka, form kosong — user tetap bisa isi manual
      });
  }

  // ── Delegasi event untuk tombol "Isi/Edit SDM" ───────────
  // Menggunakan data-* attribute — aman dari masalah special char di nama
