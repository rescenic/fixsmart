...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'tambah') {
        $n=trim($_POST['nama']??''); $u=trim($_POST['username']??''); $e=trim($_POST['email']??'');
        $p=$_POST['password']??''; $r=$_POST['role']??'user'; $d=$_POST['divisi']??''; $hp=trim($_POST['no_hp']??'');
        $valid_roles = ['user','teknisi','teknisi_ipsrs','admin'];
    ...
    elseif ($act === 'edit') {
        ...
        $valid_roles = ['user','teknisi','teknisi_ipsrs','admin'];
        if (!in_array($r, $valid_roles)) { setFlash('danger','Role tidak valid.'); redirect(APP_URL.'/pages/users.php'); }
...
$users = $pdo->query("
    ...
    FROM users u ORDER BY FIELD(u.role,'admin','teknisi','teknisi_ipsrs','user'), u.nama
")->fetchAll();
...
<div class="content">
  ...
  <div style="display:flex;gap:10px;margin-bottom:15px;flex-wrap:wrap;">
    ...
    <div style="background:#fff;border-radius:4px;padding:10px 16px;display:flex;align-items:center;gap:10px;box-shadow:0 1px 4px rgba(0,0,0,.06);min-width:130px;">
      ...
      <div>
        <div style="font-size:20px;font-weight:700;color:#333;line-height:1;"><?= $cnt_roles[$key] ?? 0 ?></div>
        <div style="font-size:11px;color:#aaa;"><?= $lbl ?></div>
      </div>
    </div>
    <?php endforeach; ?>
  </div>
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>Role</th><th>Aktivitas</th><th>Status</th><th style="text-align:center;">Aksi</th>
          </tr>
        </thead>
        <tbody>
          <?php $no=1; foreach ($users as $u):
            $rstyle = [
              'admin'        => ['#ede9fe','#7c3aed','av-purple','fa-shield-alt'],
              'teknisi'      => ['#dbeafe','#1d4ed8','av-blue',  'fa-wrench'],
              'teknisi_ipsrs'=> ['#d1fae5','#065f46','av-green', 'fa-screwdriver-wrench'],
              'user'         => ['#f3f4f6','#374151','',         'fa-user'],
            ];
          ...
          <tr>
            ...
            <td>
              <div class="d-flex ai-c gap6">
                ...
                <div>
                  <div style="font-weight:600;"><?= clean($u['nama']) ?></div>
                  <div style="font-size:11px;color:#aaa;"><?= clean($u['email']) ?></div>
                </div>
              </div>
            </td>
            ...
            <td>
              <span style="font-size:11px;font-weight:700;padding:3px 9px;border-radius:3px;background:<?= $rbg ?>;color:<?= $rc ?>;">
                <i class="fa <?= $ric ?>" style="font-size:10px;"></i> <?= $role_label ?>
              </span>
            </td>
            <td style="font-size:12px;">
              <?php if ($u['role'] === 'teknisi'): ?>
                <span style="color:var(--primary);"><i class="fa fa-wrench"></i> <?= $u['handle'] ?> tiket IT</span>
              <?php elseif ($u['role'] === 'teknisi_ipsrs'): ?>
                <span style="color:#065f46;"><i class="fa fa-screwdriver-wrench"></i> <?= $u['handle_ipsrs'] ?> tiket IPSRS</span>
              <?php else: ?>
                <span style="color:var(--blue);"><i class="fa fa-paper-plane"></i> <?= $u['req'] ?> tiket</span>
              <?php endif; ?>
            </td>
            <td>
            ...
            <td style="text-align:center;white-space:nowrap;">
              ...
              <button class="btn btn-info btn-sm" onclick="resetPwd(<?= $u['id'] ?>,'<?= clean($u['nama']) ?>')" title="Reset password">
                <i class="fa fa-key"></i>
              </button>
              <?php else: ?>
    ...
    <div class="form-row">
      <div class="form-group"><label>Role</label>
        <select name="role" class="form-control">
          ...
          <option value="teknisi">Teknisi IT — Staff IT</option>
          <option value="teknisi_ipsrs">Teknisi IPSRS — Staff Sarpras</option>
  ...
  <div class="modal-bd">
    ...
    <div class="form-group">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
        ...
        <label id="lbl-<?= $rv ?>" style="border:2px solid #e5e7eb;border-radius:5px;padding:10px;cursor:pointer;transition:all .18s;display:flex;gap:8px;align-items:flex-start;">
...
function hilightRole() {
  document.querySelectorAll('[name=role]').forEach(r => {
    ...
    if (!lbl) return;
    if (r.checked) {
      lbl.style.borderColor = roleHL[r.value]?.border || '#e5e7eb';
      lbl.style.background  = roleHL[r.value]?.bg     || '#fff';
    } else {
