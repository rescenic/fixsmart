...
$role_labels = ['admin'=>'Admin','teknisi'=>'Teknisi IT','teknisi_ipsrs'=>'Teknisi IPSRS','hrd'=>'HRD','akreditasi'=>'Tim Akreditasi','user'=>'User'];
$is_hrd   = ($_SESSION['user_role'] ?? '') === 'hrd';
$is_admin = ($_SESSION['user_role'] ?? '') === 'admin';
$assignable_roles = $is_hrd ? ['user','teknisi','teknisi_ipsrs','hrd'] : VALID_ROLES;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $act = $_POST['action'] ?? '';
    if ($act === 'tambah') {
        if ($is_hrd && !hasRole('admin')) { setFlash('danger','HRD tidak berwenang menambah akun Admin atau Tim Akreditasi.'); redirect(APP_URL.'/pages/users.php'); }
        $n=$_POST['nama']??''; $u=$_POST['username']??''; $e=$_POST['email']??''; $p=$_POST['password']??''; $r=$_POST['role']??'user';
        $n=trim($n); $u=trim($u); $e=trim($e);
    ...
    elseif ($act === 'edit') {
        $id=(int)($_POST['id']??0); $n=trim($_POST['nama']??''); $e=trim($_POST['email']??''); $r=$_POST['role']??'';
        $is_akr_flag = isset($_POST['is_akreditasi']) ? 1 : 0;
        $pokja_id = ($r==='akreditasi'||$is_akr_flag) ? ((int)($_POST['pokja_id']??0)?:null) : null;
        if ($is_hrd) {
            $tgt=$pdo->prepare("SELECT role FROM users WHERE id=?"); $tgt->execute([$id]);
            if ($tgt->fetchColumn()==='admin') { setFlash('danger','HRD tidak berwenang mengubah akun Admin.'); redirect(APP_URL.'/pages/users.php'); }
        }
        if (!in_array($r,$assignable_roles)) { setFlash('danger','Role tidak valid.'); redirect(APP_URL.'/pages/users.php'); }
        ...
        elseif ($id===(int)$_SESSION['user_id']&&$r!==$_SESSION['user_role']) { setFlash('warning','Tidak bisa mengubah role akun Anda sendiri.'); }
            ...
            if ($chk->fetch()) { setFlash('warning','Email sudah digunakan akun lain.'); }
                ...
                $pdo->prepare("UPDATE users SET nama=?,email=?,role=?,pokja_id=?,is_akreditasi=? WHERE id=?")->execute([$n,$e,$r,$pokja_id,$is_akr_flag,$id]);
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>Role</th><th>Pokja / Aktivitas</th><th>Status</th>
            <th style="text-align:center;">Aksi</th>
          </tr>
        </thead>
        <tbody>
          <?php $no=1; foreach ($users as $u):
            $role=$u['role']??'user'; $status=$u['status']??'aktif';
            $rs=$rstyle[$role]??$rstyle['user']; $rl=$role_labels[$role]??ucfirst($role);
          ...
          <tr>
            ...
            <td style="text-align:center;white-space:nowrap;">
              ...
              <button class="btn btn-warning btn-sm" onclick='editUser(<?=json_encode(["id"=>(int)$u["id"],"nama"=>$u["nama"],"email"=>$u["email"],"role"=>$u["role"],"pokja_id"=>(int)($u["pokja_id"]??0),"is_akreditasi"=>(int)($u["is_akreditasi"]??0)])?>)'>
          ...
          <?php endforeach; ?>
          <?php if (empty($users)): ?><tr><td colspan="8" style="text-align:center;color:#94a3b8;padding:30px;">Belum ada pengguna.</td></tr><?php endif; ?>
        </tbody>
      </table>
    </div>
  </div>
      ...
      <div class="modal-bd">
        ...
        <div class="form-group">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
            ...
            ?>
            <label id="albl-<?=$rv?>" style="border:2px solid #e5e7eb;border-radius:8px;padding:10px;cursor:pointer;transition:all .18s;display:flex;gap:8px;align-items:flex-start;background:<?=$checked?$rbg:'#fff'?>;">
              <input type="radio" name="role" value="<?=$rv?>" <?=$checked?'checked':''?> style="margin-top:3px;flex-shrink:0;" onchange="hilightRoleAdd();togglePokjaAdd()">
              <div>
        ...
        <div class="form-group" id="add-akreditasi-extra" style="display:none;">
          <div style="background:#fefce8;border:1px dashed #fbbf24;border-radius:8px;padding:10px 13px;">
            <label style="display:flex;align-items:flex-start;gap:8px;cursor:pointer;margin:0;">
              <input type="checkbox" name="is_akreditasi" id="add-is-akr" value="1" onchange="togglePokjaAdd()" style="width:16px;height:16px;cursor:pointer;margin-top:2px;accent-color:#d97706;">
              <div>
                <div style="font-size:12.5px;font-weight:700;color:#854d0e;"><i class="fa fa-medal" style="font-size:11px;"></i> Tambahkan akses Akreditasi</div>
                <div style="font-size:10.5px;color:#a16207;margin-top:2px;line-height:1.5;">Centang agar orang ini <strong>juga bisa input &amp; lihat dokumen akreditasi</strong>, tanpa mengganti role utamanya.</div>
              </div>
            </label>
          </div>
        </div>
        <div class="form-group" id="add-pokja-group" style="display:none;">
          ...
          <small style="color:#9ca3af;font-size:10.5px;margin-top:3px;display:block;"><i class="fa fa-info-circle"></i> Satu orang hanya bisa di satu Pokja</small>
        </div>
        <!-- Info atasan -->
        <div style="background:#f0fdf9;border:1px solid #a7f3d0;border-radius:8px;padding:10px 13px;font-size:12px;color:#065f46;display:flex;align-items:flex-start;gap:8px;margin-bottom:12px;">
          <i class="fa fa-user-tie" style="margin-top:2px;flex-shrink:0;"></i>
          <div><strong>Atasan Langsung</strong> tidak diisi di sini.<br>Setelah akun dibuat, karyawan dapat memilih sendiri atasannya melalui <strong>Profil Saya → Data Kepegawaian</strong>.</div>
        </div>
        <div class="form-group">
    ...
    <form method="POST">
      ...
      <div class="modal-bd">
        <div style="background:#fffbeb;border:1px solid #fde68a;border-radius:6px;padding:9px 12px;margin-bottom:14px;font-size:12px;color:#92400e;">
          <i class="fa fa-triangle-exclamation"></i> Mengubah <strong>Role</strong> akan langsung mengubah hak akses pengguna di seluruh sistem.
        </div>
        <div class="form-row">
          <div class="form-group">
            <label>Nama Lengkap <span class="req">*</span></label>
        ...
        <div class="form-group">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;" id="edit-role-grid">
            ...
            ?>
            <label id="lbl-<?=$rv?>" style="border:2px solid #e5e7eb;border-radius:8px;padding:10px;cursor:pointer;transition:border-color .18s,background .18s;display:flex;gap:8px;align-items:flex-start;background:#fff;">
              <input type="radio" name="role" value="<?=$rv?>" style="margin-top:3px;flex-shrink:0;" onchange="hilightRole();togglePokjaEdit()">
              <div>
                <div style="display:flex;align-items:center;gap:5px;font-size:12px;font-weight:700;">
                  <span style="width:20px;height:20px;border-radius:50%;background:<?=$rbg?>;display:inline-flex;align-items:center;justify-content:center;flex-shrink:0;">
                    <i class="fa <?=$ri?>" style="color:<?=$rc?>;font-size:9px;"></i>
        ...
        <div class="form-group" id="edit-akreditasi-extra" style="display:none;">
          <div style="background:#fefce8;border:1px dashed #fbbf24;border-radius:8px;padding:10px 13px;">
            <label style="display:flex;align-items:flex-start;gap:8px;cursor:pointer;margin:0;">
              <input type="checkbox" name="is_akreditasi" id="edit-is-akr" value="1" onchange="togglePokjaEdit()" style="width:16px;height:16px;cursor:pointer;margin-top:2px;accent-color:#d97706;">
              <div>
                <div style="font-size:12.5px;font-weight:700;color:#854d0e;"><i class="fa fa-medal" style="font-size:11px;"></i> Tambahkan akses Akreditasi</div>
                <div style="font-size:10.5px;color:#a16207;margin-top:2px;line-height:1.5;">Centang agar orang ini <strong>juga bisa input &amp; lihat dokumen akreditasi</strong>, tanpa mengganti role utama.</div>
              </div>
            </label>
          </div>
        </div>
        ...
        <!-- Info atasan -->
        <div style="background:#f0fdf9;border:1px solid #a7f3d0;border-radius:8px;padding:10px 13px;font-size:12px;color:#065f46;display:flex;align-items:flex-start;gap:8px;">
          <i class="fa fa-user-tie" style="margin-top:2px;flex-shrink:0;"></i>
          <div><strong>Atasan Langsung</strong> diatur mandiri oleh karyawan.<br>Karyawan dapat memilih atasan mereka di <strong>Profil Saya → Data Kepegawaian → Kepegawaian</strong>.</div>
        </div>
      </div>
      <div class="modal-ft">
...
<script>
const roleColors = {
  user:{border:'#d1d5db',bg:'#f9fafb'},teknisi:{border:'#93c5fd',bg:'#eff6ff'},
  teknisi_ipsrs:{border:'#6ee7b7',bg:'#f0fdf4'},hrd:{border:'#f9a8d4',bg:'#fdf2f8'},
  akreditasi:{border:'#fde047',bg:'#fefce8'},admin:{border:'#c4b5fd',bg:'#faf5ff'},
};
function editUser(u){
  ...
  let found=false;
  document.querySelectorAll('#m-edit [name=role]').forEach(function(r){r.checked=(r.value===u.role);if(r.checked)found=true;});
  if(!found){const fb=document.querySelector('#m-edit [name=role][value=user]');if(fb)fb.checked=true;}
  hilightRole();
  ...
  if(pokjaSel&&u.pokja_id)for(let i=0;i<pokjaSel.options.length;i++)pokjaSel.options[i].selected=(pokjaSel.options[i].value==u.pokja_id);
  openModal('m-edit');
}
function hilightRole(){
  document.querySelectorAll('#m-edit [name=role]').forEach(function(r){
    const lbl=document.getElementById('lbl-'+r.value);if(!lbl)return;
    const c=roleColors[r.value]||{border:'#e5e7eb',bg:'#fff'};
    lbl.style.borderColor=r.checked?c.border:'#e5e7eb';lbl.style.background=r.checked?c.bg:'#fff';
  });
}
function hilightRoleAdd(){
  document.querySelectorAll('#m-add [name=role]').forEach(function(r){
    const lbl=document.getElementById('albl-'+r.value);if(!lbl)return;
    const c=roleColors[r.value]||{border:'#e5e7eb',bg:'#fff'};
    lbl.style.borderColor=r.checked?c.border:'#e5e7eb';lbl.style.background=r.checked?c.bg:'#fff';
  });
}
...
function togglePokjaEdit(){
  ...
  const isAkr=rc&&rc.value==='akreditasi';
  ...
  if(ex)ex.style.display=isAkr?'none':'block';
  if(grp)grp.style.display=(isAkr||(chk&&chk.checked))?'block':'none';
  if(isAkr&&chk)chk.checked=false;
}
