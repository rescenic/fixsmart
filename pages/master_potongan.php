...
$page_title  = 'Master Potongan';
$active_menu = 'master_potongan';

// ── Auto-create tabel ─────────────────────────────────────────────────────────
try {
    $pdo->exec("CREATE TABLE IF NOT EXISTS `master_potongan` (
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'tambah' || $act === 'edit') {
        ...
        if (!$nama || !$kode) {
            ...
            try {
                if ($act === 'tambah') {
                    $chk = $pdo->prepare("SELECT COUNT(*) FROM master_potongan WHERE kode=?");
                    $chk->execute([$kode]);
                    if ($chk->fetchColumn() > 0) {
                        setFlash('danger', "Kode <strong>$kode</strong> sudah digunakan.");
                    } else {
                        $pdo->prepare("INSERT INTO master_potongan
                        ...
                        setFlash('success', 'Komponen potongan berhasil ditambahkan.');
                    }
                } else {
                    $id = (int)($_POST['id'] ?? 0);
                    $pdo->prepare("UPDATE master_potongan
                    ...
                    setFlash('success', 'Komponen potongan berhasil diperbarui.');
                }
            } catch (Exception $e) {
                setFlash('danger', 'Gagal menyimpan: ' . htmlspecialchars($e->getMessage()));
            }
        }
        redirect(APP_URL . '/pages/master_potongan.php');
    }
    ...
    if ($act === 'hapus') {
        ...
        $used = 0;
        try {
            $cu = $pdo->prepare("SELECT COUNT(*) FROM data_gaji_detail WHERE potongan_id=?");
            $cu->execute([$id]);
        ...
        if ($used > 0) {
            ...
            $pdo->prepare("DELETE FROM master_potongan WHERE id=?")->execute([$id]);
            setFlash('success', 'Komponen potongan berhasil dihapus.');
        }
        redirect(APP_URL . '/pages/master_potongan.php');
    }
    ...
    if ($act === 'toggle_aktif') {
        $id  = (int)($_POST['id'] ?? 0);
        $val = (int)($_POST['val'] ?? 0);
        $pdo->prepare("UPDATE master_potongan SET aktif=? WHERE id=?")->execute([$val,$id]);
...
$stmt   = $pdo->prepare("SELECT * FROM master_potongan $where ORDER BY kode ASC");
$stmt->execute($params);
...
<div class="page-header">
  <h4><i class="fa fa-circle-minus" style="color:#ef4444;"></i> &nbsp;Master Potongan</h4>
  <div class="breadcrumb">
    <a href="<?= APP_URL ?>/dashboard.php">Dashboard</a>
    <span class="sep">/</span>
    <span class="cur">Master Potongan</span>
  </div>
</div>
...
<div class="content">
    ...
    <div class="panel-bd">
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;">
        <form method="GET" style="display:flex;gap:8px;align-items:center;">
          ...
          <?php if ($search): ?><a href="master_potongan.php" class="btn btn-default" style="height:34px;">Reset</a><?php endif; ?>
        </form>
        <button onclick="openModal('m-tambah')" class="btn btn-danger">
  ...
  <div class="panel">
    <div class="panel-hd">
      <h5><i class="fa fa-circle-minus" style="color:#ef4444;"></i> &nbsp;Daftar Komponen Potongan
    ...
    <div class="panel-bd np tbl-wrap">
      <table>
        ...
        <tbody>
          <?php if (!$rows): ?>
          <tr><td colspan="7" class="td-empty"><i class="fa fa-circle-minus"></i> Belum ada komponen potongan</td></tr>
          <?php else: foreach ($rows as $i => $r): ?>
          <tr>
            <td style="color:#bbb;font-size:12px;"><?= $i+1 ?></td>
            <td>
              <code style="background:#fef2f2;color:#9f1239;padding:2px 8px;border-radius:5px;font-size:12px;font-weight:700;">
            ...
            <td style="text-align:right;font-weight:500;color:#991b1b;">
            ...
            <td style="text-align:center;white-space:nowrap;">
              <button onclick="editPotongan(<?= htmlspecialchars(json_encode($r), ENT_QUOTES) ?>)" class="btn btn-primary btn-sm" title="Edit">
                <i class="fa fa-pen"></i>
              </button>
              <button onclick="hapusPotongan(<?= $r['id'] ?>, '<?= addslashes(htmlspecialchars($r['nama'])) ?>')" class="btn btn-danger btn-sm" title="Hapus">
...
<div class="modal-ov" id="m-tambah">
  <div style="background:#fff;width:100%;max-width:480px;border-radius:14px;box-shadow:0 20px 60px rgba(0,0,0,.2);animation:mIn .2s ease;overflow:hidden;">
    <div style="padding:15px 20px;border-bottom:1px solid #e5e7eb;display:flex;align-items:center;justify-content:space-between;">
      <div style="font-size:14px;font-weight:700;"><i class="fa fa-circle-minus" style="color:#ef4444;"></i> Tambah Komponen Potongan</div>
      ...
      <div style="padding:20px;display:flex;flex-direction:column;gap:12px;">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
          <div>
            ...
            <input type="text" name="kode" placeholder="cth: BPJS" maxlength="20" required class="form-control" style="text-transform:uppercase;">
          </div>
          <div>
        ...
        <div>
          ...
          <input type="text" name="nama" placeholder="cth: BPJS Kesehatan" required class="form-control">
        </div>
        <div>
...
<div class="modal-ov" id="m-edit">
  <div style="background:#fff;width:100%;max-width:480px;border-radius:14px;box-shadow:0 20px 60px rgba(0,0,0,.2);animation:mIn .2s ease;overflow:hidden;">
    <div style="padding:15px 20px;border-bottom:1px solid #e5e7eb;display:flex;align-items:center;justify-content:space-between;">
      <div style="font-size:14px;font-weight:700;"><i class="fa fa-pen" style="color:#3b82f6;"></i> Edit Komponen Potongan</div>
...
function hapusPotongan(id, nama) {
